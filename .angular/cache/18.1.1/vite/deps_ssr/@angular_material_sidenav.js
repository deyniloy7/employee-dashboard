...
var DateAdapter = class {
  constructor() {
    this._localeChanges = new import_rxjs6.Subject();
...
var _MatOption = class _MatOption {
  ...
  constructor(_element, _changeDetectorRef, _parent, group) {
    ...
    this._stateChanges = new import_rxjs6.Subject();
  }
  /**
...
var _MatDrawer = class _MatDrawer {
  ...
  constructor(_elementRef, _focusTrapFactory, _focusMonitor, _platform, _ngZone, _interactivityChecker, _doc, _container) {
    ...
    this._opened = false;
    this._animationStarted = new import_rxjs7.Subject();
    this._animationEnd = new import_rxjs7.Subject();
    ...
    this._openedStream = this.openedChange.pipe((0, import_operators6.filter)((o) => o), (0, import_operators6.map)(() => {
    }));
    this.openedStart = this._animationStarted.pipe((0, import_operators6.filter)((e) => e.fromState !== e.toState && e.toState.indexOf("open") === 0), (0, import_operators6.mapTo)(void 0));
    this._closedStream = this.openedChange.pipe((0, import_operators6.filter)((o) => !o), (0, import_operators6.map)(() => {
    }));
    this.closedStart = this._animationStarted.pipe((0, import_operators6.filter)((e) => e.fromState !== e.toState && e.toState === "void"), (0, import_operators6.mapTo)(void 0));
    this._destroyed = new import_rxjs7.Subject();
    this.onPositionChanged = new EventEmitter();
    this._modeChanged = new import_rxjs7.Subject();
    this._injector = inject(Injector);
    this._changeDetectorRef = inject(ChangeDetectorRef);
    this.openedChange.pipe((0, import_operators6.takeUntil)(this._destroyed)).subscribe((opened) => {
      if (opened) {
        if (this._doc) {
    ...
    this._ngZone.runOutsideAngular(() => {
      (0, import_rxjs7.fromEvent)(this._elementRef.nativeElement, "keydown").pipe((0, import_operators6.filter)((event) => {
        return event.keyCode === ESCAPE && !this.disableClose && !hasModifierKey(event);
      }), (0, import_operators6.takeUntil)(this._destroyed)).subscribe((event) => this._ngZone.run(() => {
        this.close();
        event.stopPropagation();
        event.preventDefault();
      }));
    });
    this._animationEnd.pipe((0, import_operators6.distinctUntilChanged)((x, y) => {
  ...
  _setOpen(isOpen, restoreFocus, focusOrigin) {
    ...
    this._updateFocusTrapState();
    return new Promise((resolve) => {
      this.openedChange.pipe((0, import_operators6.take)(1)).subscribe((open) => resolve(open ? "open" : "close"));
    });
  }
  _getWidth() {
...
var _MatDrawerContainer = class _MatDrawerContainer {
  ...
  constructor(_dir, _element, _ngZone, _changeDetectorRef, viewportRuler, defaultAutosize = false, _animationMode) {
    ...
    this._destroyed = new import_rxjs7.Subject();
    this._doCheckSubject = new import_rxjs7.Subject();
    this._contentMargins = {
      left: null,
      right: null
    };
    this._contentMarginChanges = new import_rxjs7.Subject();
    this._injector = inject(Injector);
    if (_dir) {
    ...
    viewportRuler.change().pipe((0, import_operators6.takeUntil)(this._destroyed)).subscribe(() => this.updateContentMargins());
    this._autosize = defaultAutosize;
  }
  ngAfterContentInit() {
    this._allDrawers.changes.pipe((0, import_operators6.startWith)(this._allDrawers), (0, import_operators6.takeUntil)(this._destroyed)).subscribe((drawer) => {
      this._drawers.reset(drawer.filter((item) => !item._container || item._container === this));
      this._drawers.notifyOnChanges();
    });
    this._drawers.changes.pipe((0, import_operators6.startWith)(null)).subscribe(() => {
      this._validateDrawers();
    ...
    this._ngZone.runOutsideAngular(() => {
      this._doCheckSubject.pipe(
        (0, import_operators6.debounceTime)(10),
        // Arbitrary debounce time, less than a frame at 60fps
        (0, import_operators6.takeUntil)(this._destroyed)
  ...
  _watchDrawerToggle(drawer) {
    drawer._animationStarted.pipe((0, import_operators6.filter)((event) => event.fromState !== event.toState), (0, import_operators6.takeUntil)(this._drawers.changes)).subscribe((event) => {
    ...
    if (drawer.mode !== "side") {
      drawer.openedChange.pipe((0, import_operators6.takeUntil)(this._drawers.changes)).subscribe(() => this._setContainerClass(drawer.opened));
    }
  }
  /**
  ...
  _watchDrawerPosition(drawer) {
    ...
    drawer.onPositionChanged.pipe((0, import_operators6.takeUntil)(this._drawers.changes)).subscribe(() => {
      afterNextRender(() => {
        this._validateDrawers();
      }, {
        injector: this._injector,
  ...
  _watchDrawerMode(drawer) {
    if (drawer) {
      drawer._modeChanged.pipe((0, import_operators6.takeUntil)((0, import_rxjs7.merge)(this._drawers.changes, this._destroyed))).subscribe(() => {
        this.updateContentMargins();
        this._changeDetectorRef.markForCheck();
      });
    }
